...
def create_visualization(models_dict, raw_slice, subject_name, output_path):
    """
    Create comprehensive visualization with raw data + all three layer models.
    
    Args:
        models_dict: Dict with '3layer', '6layer', '9layer' models
        raw_slice: Raw BrainWeb slice data
        subject_name: Name for title
        output_path: Path to save figure
    """
    fig, axes = plt.subplots(1, 4, figsize=(24, 6))
    
    # Color schemes for layer models
    ...
    # Plot 0: Raw BrainWeb data
    im0 = axes[0].imshow(raw_slice.T, cmap='gray', origin='lower')
    axes[0].set_title('Raw BrainWeb Data', fontsize=14, fontweight='bold')
    axes[0].axis('equal')
    axes[0].axis('off')
    cbar0 = plt.colorbar(im0, ax=axes[0])
    cbar0.set_label('Tissue Type', fontweight='bold')
    
    # Plot 1: 3-layer
    im1 = axes[1].imshow(models_dict['3layer'].T, cmap=cmap_3, norm=norm_3, origin='lower')
    axes[1].set_title('3-Layer Model', fontsize=14, fontweight='bold')
    axes[1].axis('equal')
    axes[1].axis('off')
    cbar1 = plt.colorbar(im1, ax=axes[1], ticks=range(4))
    cbar1.set_ticklabels(['BG', 'Scalp', 'Skull', 'Brain'])
    
    # Plot 2: 6-layer
    im2 = axes[2].imshow(models_dict['6layer'].T, cmap=cmap_6, norm=norm_6, origin='lower')
    axes[2].set_title('6-Layer Model', fontsize=14, fontweight='bold')
    axes[2].axis('equal')
    axes[2].axis('off')
    cbar2 = plt.colorbar(im2, ax=axes[2], ticks=range(7))
    cbar2.set_ticklabels(['BG', 'Scalp', 'Skull', 'CSF', 'Grey', 'White', 'Vent'])
    
    # Plot 3: 9-layer
    im3 = axes[3].imshow(models_dict['9layer'].T, cmap=cmap_9, norm=norm_9, origin='lower')
    axes[3].set_title('9-Layer Model', fontsize=14, fontweight='bold')
    axes[3].axis('equal')
    axes[3].axis('off')
    cbar3 = plt.colorbar(im3, ax=axes[3], ticks=range(10))
...
def process_single_subject(subject_file, subject_idx, output_base_dir='brainweb_subjects'):
    ...
    # Create visualization with raw data
    models_dict = {
        '3layer': model_3,
        '6layer': model_6,
        '9layer': model_9
    ...
    viz_path = subject_dir / 'models_viz.png'
    create_visualization(models_dict, slice_2d, subject_name, viz_path)  # Added raw_slice
