...
def load_brainweb_mesh(
    ...
    # ---- NEW: restrict boundary facets to scalp only ----
    all_boundary = mesh.boundary_facets()          # all facets with only one cell
    scalp_facets = []

    scalp_id = 1  # change if your BrainWeb labeling is different

    for f in all_boundary:
        # f2t is (2, n_facets); -1 means "no triangle"
        tri_candidates = mesh.f2t[:, f]
        tri = tri_candidates[tri_candidates >= 0][0]   # the unique adjacent triangle
    ...
    # Compute facet midpoints and angles only on scalp boundary
    facet_data = []
...
# def demo():
#     """Demo matching eit_fenicsx usage."""
#     print("="*70)
