...
function obj = objective_function(step_size, sigma, del_sigma, rec_fmdl, V_measured)
    % Compute the objective function value for a given step size
    % This evaluates the data misfit after taking a step
    
    % Compute trial conductivity
    sigma_trial = sigma + step_size * del_sigma;
    
    % Solve forward problem with trial conductivity
    img_trial = mk_image(rec_fmdl, sigma_trial);
    U_trial = fwd_solve(img_trial);
    
    % Compute data misfit (L2 norm)
    obj = norm(U_trial.meas(:) - V_measured(:))^2;
end